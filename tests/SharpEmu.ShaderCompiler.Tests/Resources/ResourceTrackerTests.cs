...
public sealed class ResourceTrackerTests
{
    ...
    [Fact]
    public void ComputeWorkgroupIdSgprIsASystemValueNotUndefined()
    {
        var program = Program(
            MoveScalar(0, 4, 0),
            MoveScalar(4, 5, 0),
            MoveScalar(8, 6, 64),
            MoveScalar(12, 7, 0),
            ScalarBufferLoad(16, 4, destination: 8, dynamicOffsetRegister: 2),
            EndProgram(20));

        var withSystem = ScalarValueGraph.Build(program, 0, 2, new Gen5ComputeSystemRegisters(2, null, null, null));
