	...
	private unsafe TlsPatchCounts PatchTlsPatternsInRange(ulong rangeStart, ulong rangeEnd)
	{
		...
		while (num < num2)
		{
			...
			if (flag && flag2 && num6 > num5 + MinTlsPatchInstructionBytes)
			{
				...
				// TLS and stack-canary patterns must be recognized at instruction
				// boundaries. The old byte-at-a-time scan could match the bytes of
				// an unrelated instruction (or an embedded immediate), overwrite
				// that instruction with a call/NOP sequence, and leave GTA with
				// corrupted code before its first real frame. Kyty made this exact
				// change in its runtime linker by advancing a Zydis decoder one
				// instruction at a time; use Iced here, which is already SharpEmu's
				// native x64 decoder.
				...
				while (reader.Position < scanBytes)
				{
					...
					{
						// Keep scanning after malformed/data bytes. This mirrors the
						// loader's existing tolerant behavior without reintroducing
						// byte-offset pattern matches inside a decoded instruction.
							...
							instructionOffset))
					{
						...
						TryPatchTlsImmediateStoreInstruction(address, ptr + instructionOffset))
					{
						num9++;
					}
					else if (remainingBytes >= 12 &&
						TryPatchSse4aExtrqBlend(address, ptr + instructionOffset))
					{
						sse4aPatchCount++;
					}
